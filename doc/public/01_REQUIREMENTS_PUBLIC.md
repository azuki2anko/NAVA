...
対象: Windows 11 x64 / Yamaha RX-V4A

## 1. 目的

同一LAN上のYamaha RX-V4Aを、Windows 11のGUI、タスクトレイ、グローバルホットキー、型付きlocalhost APIから安全に管理する公開可能なデスクトップアプリを提供する。

公開版は、個人環境固有の接続先、識別子、マクロ番号、外部アプリ名、秘密情報を含まない。実機Capabilityに応じて利用可能な機能だけを提示する。
...
- Yamaha Extended Control APIによるRX-V4Aの検出、状態取得、制御
...
- PUB-FR-107: Yamaha応答コード成功かつ対象モデルであることを検証する。
...
- PUB-FR-203: 入力、音量、ミュート、音場、信号情報、SCENEをCapabilityに応じて表示・操作する。
...
- PUB-NFR-03: RX-V4A不在、IP変更、再起動、NIC変更でアプリが停止しない。
...
- PUB-SEC-01: RX-V4Aの認証なしHTTP APIをインターネットへ公開しない。
